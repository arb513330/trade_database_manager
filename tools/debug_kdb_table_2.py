...
def get_bar_data(barlib, limit_lib, symbol, exchange, interval="1d", start=None, end=None):
    ...
    if interval == "1d":
        ...
        df = pd.merge(bar_df, limit_df, left_index=True, right_index=True, how="left").drop(columns=["order_book_id", "open_interest"], errors="ignore")
...
if __name__ == "__main__":
    ...
    for ticker, exch in all_cn_stock.index.values[0:1000]:
        ...
        try:
            ...
            kdb_mgr.write_partitioned(f"i1m", df, path="stk1", partition_func=partial(_get_partition_bucket, bar_freq="1m"), key_column="datetime")
